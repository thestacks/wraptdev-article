namespace BooksManagement.IntegrationTests;
...
public class TestFixture
{
    ...
    public static Task ExecuteDbContextAsync(Func<BooksDbContext, Task> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>()));

    public static Task ExecuteDbContextAsync(Func<BooksDbContext, ValueTask> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>()).AsTask());

    public static Task ExecuteDbContextAsync(Func<BooksDbContext, IMediator, Task> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>(), sp.GetService<IMediator>()));

    public static Task<T> ExecuteDbContextAsync<T>(Func<BooksDbContext, Task<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>()));

    public static Task<T> ExecuteDbContextAsync<T>(Func<BooksDbContext, ValueTask<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>()).AsTask());

    public static Task<T> ExecuteDbContextAsync<T>(Func<BooksDbContext, IMediator, Task<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<BooksDbContext>(), sp.GetService<IMediator>()));
